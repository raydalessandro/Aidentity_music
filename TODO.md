...
| 7 | **Il modulo dei contatti si rifiuta senza dirlo** | Misurato nei log del progetto ospitato: `POST /rest/v1/site_contacts` compare **zero volte**, mentre nella stessa sessione `site_links`, `site_metrics`, `site_posts` e `site_tracks` hanno tutti risposto `201`. Quindi non è RLS, non è un vincolo, non è il server: la richiesta **non è mai partita dal browser**. Il modulo dei contatti è l'unico con `required` più `type="email"`, cioè l'unico dove la validazione nativa può rifiutare il `submit`; e su un telefono quel rifiuto è spesso invisibile, perché il fumetto del browser non compare se il campo non è a schermo. Chi scrive vede un tasto che non fa niente. Chiusa quando la validazione dei moduli EPK vive nell'applicazione e scrive nell'area errori che già esiste, invece di delegare al browser un rifiuto muto — con un test che dimostra che un indirizzo non valido produce un messaggio visibile e non il silenzio. |
